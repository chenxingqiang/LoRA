...
parser.add_argument('--lora_dim', type=int, default=0,
                    help='lora attn dimension')

parser.add_argument('--lora_alpha', type=int,
                    default=128, help='lora attn alpha')
...
parser.add_argument('--no_repeat_ngram_size', type=int,
                    default=4, help='no_repeat_ngram_size')

parser.add_argument('--repetition_penalty', type=float,
                    default=1.0, help='repetition_penalty')
...
def beam(model, data_iter, args):
    ...
    with torch.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            _batch = torch.arange(0, _id.size(
                0), device=args.device, dtype=torch.long)

            past = None
            len_past = None

            _query = _query.repeat(1, num_beams).view(
            ...
            with torch.no_grad():
                for i in range(0, args.eval_len):
                    if i == 0:
                        ...
                        logits, past = model(
                            token_id, past=past, len_past=len_past)
                    ...
                    if i == 0:
                        next_scores = _logprob.view(
                            batch_size, num_beams, -1)[:, 0, :]  # batch_size, vocab

                    else:
                        next_scores = beam_scores.unsqueeze(
                            -1) + _logprob.view(batch_size, num_beams, -1)
                        next_scores = next_scores.view(
                            batch_size, -1)  # batch_size, beam * vocab
                    ...
                    # batch_size * num_beams
                    beam_id = (next_tokens // vocab_size).view(-1)
                    # batch_size, num_beams
                    token_id = (next_tokens %
                                vocab_size).view(-1).unsqueeze(-1)

                    beam_idx = beam_id.view(
                        batch_size, num_beams) + (_batch * num_beams).unsqueeze(-1)
                    ...
                    if history is None:
                        ...
                        history = torch.cat(
                            (history[beam_idx.view(-1)], token_id.detach()), dim=1).detach()
