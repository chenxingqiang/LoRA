...
class FunnelModelTester:
    ...
    def __init__(
        ...
        self.num_hidden_layers = sum(
            self.block_sizes) + (0 if base else self.num_decoder_layers)
    ...
    def create_and_check_model(
        ...
        self.parent.assertEqual(
            result.last_hidden_state.shape, (self.batch_size, self.seq_length, self.d_model))
    ...
    def create_and_check_for_pretraining(
        ...
        model.eval()
        result = model(input_ids, attention_mask=input_mask,
                       token_type_ids=token_type_ids, labels=fake_token_labels)
...
class FunnelBaseModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    test_pruning = False
    all_model_classes = (
        (FunnelBaseModel, FunnelForMultipleChoice,
         FunnelForSequenceClassification) if is_torch_available() else ()
    )
...
class FunnelModelIntegrationTest(unittest.TestCase):
    def test_inference_tiny_model(self):
        ...
        input_ids = torch.arange(
            0, batch_size * sequence_length).long().reshape(batch_size, sequence_length)
