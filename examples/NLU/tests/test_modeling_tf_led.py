...
class TFLEDModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        inputs_dict["global_attention_mask"] = tf.where(
            tf.range(self.model_tester.seq_length)[
                None, :] < num_global_attn_indices,
            1,
        ...
        def check_encoder_attentions_output(outputs):
            ...
            global_attentions = [t.numpy()
                                 for t in outputs.encoder_global_attentions]
            self.assertEqual(
            ...
            self.assertListEqual(
                ...
                [self.model_tester.num_attention_heads,
                 encoder_seq_length, num_global_attn_indices],
            )
...
class TFLEDModelIntegrationTest(unittest.TestCase):
    def test_inference_no_head(self):
        ...
        expected_slice = tf.convert_to_tensor(
            [[2.3050, 2.8279, 0.6531], [-1.8457, -0.1455, -3.5661],
    ...
    def test_inference_with_head(self):
        ...
        # change to expected output here
        expected_slice = tf.convert_to_tensor(
            [[33.6507, 6.4572, 16.8089], [5.8739, -2.4238,
                                          11.2902], [-3.2139, -4.3149, 4.2783]],
        )
        tf.debugging.assert_near(
            output[:, :3, :3], expected_slice, atol=TOLERANCE)
