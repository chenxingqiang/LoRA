...
class LEDModelTester:
    ...
    def check_global_attention(self, config, inputs_dict):
        ...
        attention_mask = ids_tensor(
            inputs_dict["input_ids"].shape, vocab_size=2)
        ...
        self.parent.assertTrue(
            ...
            (self.batch_size, self.num_attention_heads,
             self.encoder_seq_length, num_tokens_with_global_attention),
        )
...
class LEDModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        (LEDModel, LEDForConditionalGeneration,
         LEDForSequenceClassification, LEDForQuestionAnswering)
        if is_torch_available()
        else ()
    )
    ...
    def _check_encoder_attention_for_generate(self, attentions, batch_size, config, seq_length):
        ...
        tgt_length = (
            seq_length // config.attention_window[0] +
            (seq_length % config.attention_window[0] != 0)
        ) * config.attention_window[0]

        encoder_expected_shape = (
            batch_size, config.num_attention_heads, tgt_length, seq_length)
...
class LEDModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        model = LEDModel.from_pretrained(
            "allenai/led-base-16384").to(torch_device)

        # change to intended input
        input_ids = _long_tensor(
            [512 * [0, 31414, 232, 328, 740, 1140, 12695, 69]])
    ...
    def test_seq_to_seq_generation(self):
        ...
        hf = LEDForConditionalGeneration.from_pretrained(
            "allenai/led-large-16384-arxiv").to(torch_device)
