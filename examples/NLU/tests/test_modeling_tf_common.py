...
class TFModelTesterMixin:
    ...
    def _prepare_for_class(self, inputs_dict, model_class, return_labels=False) -> dict:
        ...
        if model_class in TF_MODEL_FOR_MULTIPLE_CHOICE_MAPPING.values():
            inputs_dict = {
                k: tf.tile(tf.expand_dims(
                    v, 1), (1, self.model_tester.num_choices) + (1,) * (v.ndim - 1))
                ...
                *TF_MODEL_FOR_SEQ_TO_SEQ_CAUSAL_LM_MAPPING.values(),
                ...
                inputs_dict["labels"] = tf.zeros(
                    (self.model_tester.batch_size,
                     self.model_tester.seq_length), dtype=tf.int32
                )
        return inputs_dict
    ...
    def test_onnx_runtime_optimize(self):
        ...
        for model_class in self.all_model_classes:
            ...
            onnx_model = keras2onnx.convert_keras(
                model, model.name, target_opset=self.onnx_min_opset)
    ...
    def test_keras_save_load(self):
        ...
        for main_layer_class in tf_main_layer_classes:
            ...
            model = tf.keras.Model(
                symbolic_inputs, outputs=main_layer(symbolic_inputs))
    ...
    def test_pt_tf_model_equivalence(self):
        ...
        for model_class in self.all_model_classes:
            ...
            tf_model = transformers.load_pytorch_model_in_tf2_model(
                tf_model, pt_model, tf_inputs=self._prepare_for_class(
                    inputs_dict, model_class)
            )
            ...
            for name, key in self._prepare_for_class(inputs_dict, model_class).items():
                if type(key) == bool:
                    ...
                    pt_inputs_dict[name] = torch.from_numpy(
                        key.numpy()).to(torch.long)
            ...
            tfo = tf_model(self._prepare_for_class(
                inputs_dict, model_class), training=False)
        ...
        for main_layer_class in tf_main_layer_classes:
            ...
            if "T5" in main_layer_class.__name__:
                ...
                shared = TFSharedEmbeddings(
                    self.model_tester.vocab_size, self.model_tester.hidden_size, name="shared")
                config.use_cache = False
                main_layer = main_layer_class(config, embed_tokens=shared)
            else:
                main_layer = main_layer_class(config)
            ...
            X = tf.data.Dataset.from_tensor_slices(
                (inputs_dict, np.ones((self.model_tester.batch_size,
                 self.model_tester.seq_length, num_labels, 1)))
            ).batch(1)

            hidden_states = main_layer(symbolic_inputs)[0]
            outputs = tf.keras.layers.Dense(
                num_labels, activation="softmax", name="outputs")(hidden_states)
            model = tf.keras.models.Model(
                inputs=symbolic_inputs, outputs=[outputs])
    ...
    def test_compile_tf_model(self):
        ...
        for model_class in self.all_model_classes:
            if self.is_encoder_decoder:
                ...
                input_ids = tf.keras.Input(batch_shape=(
                    4, 2, max_input), name="input_ids", dtype="int32")
            else:
                input_ids = tf.keras.Input(batch_shape=(
                    2, max_input), name="input_ids", dtype="int32")
            ...
            extended_model = tf.keras.Model(
                inputs=[input_ids], outputs=[outputs])
    ...
    def test_keyword_and_dict_args(self):
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertLess(
                np.sum(np.abs(output_dict - output_keywords)), 1e-6)
    ...
    def test_attention_outputs(self):
        ...
        for model_class in self.all_model_classes:
            ...
            if self.is_encoder_decoder:
                model = model_class(config)
                outputs = model(self._prepare_for_class(
                    inputs_dict, model_class))
                self.assertEqual(config.output_hidden_states, False)
    ...
    def test_headmasking(self):
        ...
        for model_class in self.all_model_classes:
            ...
            def prepare_layer_head_mask(i, attention_heads, num_hidden_layers):
                if i == 0:
                    return tf.concat(
                        (tf.zeros(1, dtype=tf.float32), tf.ones(
                            attention_heads - 1, dtype=tf.float32)), 0
                    )
                elif i == num_hidden_layers - 1:
                    return tf.concat(
                        (tf.zeros(attention_heads - 1, dtype=tf.float32),
                         tf.ones(1, dtype=tf.float32)), 0
                    )
                else:
            ...
            def check_attentions_validity(attentions):
                ...
                self.assertAlmostEqual(tf.math.reduce_sum(
                    attentions[0][..., 0, :, :]).numpy(), 0.0)
                self.assertNotEqual(tf.math.reduce_sum(
                    attentions[0][..., -1, :, :]).numpy(), 0.0)
                if len(attentions) > 2:  # encoder-decodere models have only 2 layers in each modules
                    self.assertNotEqual(tf.math.reduce_sum(
                        attentions[1][..., 0, :, :]).numpy(), 0.0)
                self.assertAlmostEqual(tf.math.reduce_sum(
                    attentions[-1][..., -2, :, :]).numpy(), 0.0)
                self.assertNotEqual(tf.math.reduce_sum(
                    attentions[-1][..., -1, :, :]).numpy(), 0.0)
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(config, inputs_dict, model_class):
            ...
            if model.config.is_encoder_decoder:
                ...
                self.assertEqual(len(encoder_hidden_states),
                                 expected_num_layers)
                self.assertListEqual(
                    list(encoder_hidden_states[0].shape[-2:]),
                    [self.model_tester.seq_length, self.model_tester.hidden_size],
                )
                self.assertEqual(len(decoder_hidden_states),
                                 expected_num_layers)
                self.assertListEqual(
        ...
        for model_class in self.all_model_classes:
            ...
            first, second = (
                model(self._prepare_for_class(
                    inputs_dict, model_class), training=False)[0],
                model(self._prepare_for_class(
                    inputs_dict, model_class), training=False)[0],
            )
            out_1 = first.numpy()
    ...
    def test_model_outputs_equivalence(self):
        ...
        def check_equivalence(model, tuple_inputs, dict_inputs, additional_kwargs={}):
            ...
            dict_output = model(dict_inputs, return_dict=True,
        ...
        for model_class in self.all_model_classes:
            ...
            dict_inputs = self._prepare_for_class(
                inputs_dict, model_class, return_labels=True)
            check_equivalence(
                model, tuple_inputs, dict_inputs, {
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if config.bos_token_id is None:
                ...
                self._check_generated_ids(
                    model.generate(input_ids, do_sample=True))
            else:
                # num_return_sequences = 1
                self._check_generated_ids(
                    model.generate(do_sample=True, max_length=5))
            ...
            self._check_generated_ids(model.generate(
                input_ids, do_sample=True, num_return_sequences=2))

            # check bad words tokens language generation
            # create list of 1-seq bad token and list of 2-seq of bad tokens
            bad_words_ids = [self._generate_random_bad_tokens(
                1, model), self._generate_random_bad_tokens(2, model)]
            output_tokens = model.generate(
                input_ids, do_sample=True, bad_words_ids=bad_words_ids, num_return_sequences=2
            )
            # only count generated tokens
            generated_ids = output_tokens[:, input_ids.shape[-1]:]
            self.assertFalse(self._check_match_tokens(
                generated_ids.numpy().tolist(), bad_words_ids))
    ...
    def test_lm_head_model_random_beam_search_generate(self):
        ...
        for model_class in self.all_generative_model_classes:
            ...
            if config.bos_token_id is None:
                ...
                self._check_generated_ids(model.generate(
                    input_ids, do_sample=True, num_beams=2))
            else:
                # num_return_sequences = 1
                self._check_generated_ids(model.generate(
                    do_sample=True, max_length=5, num_beams=2))

            with self.assertRaises(AssertionError):
                # generating more sequences than having beams leads is not possible
                model.generate(input_ids, do_sample=False,
                               num_return_sequences=3, num_beams=2)
        ...
        for model_class in self.all_model_classes:
            ...
            if getattr(model, "compute_loss", None):
                ...
                added_label = prepared_for_class[
                    sorted(list(prepared_for_class.keys() -
                           inputs_dict.keys()), reverse=True)[0]
                ]
    ...
    def _generate_random_bad_tokens(self, num_bad_tokens, model):
        ...
        while len(bad_tokens) < num_bad_tokens:
            token = tf.squeeze(ids_tensor(
                (1, 1), self.model_tester.vocab_size), 0).numpy()[0]
...
def ids_tensor(shape, vocab_size, rng=None, name=None, dtype=None):
    ...
    output = tf.constant(values, shape=shape,
...
class UtilsFunctionsTest(unittest.TestCase):
    ...
    def test_top_k_top_p_filtering(self):
        ...
        non_inf_expected_idx = tf.convert_to_tensor(
            [[0, 0], [0, 9], [0, 10], [0, 25], [0, 26], [
                1, 13], [1, 17], [1, 18], [1, 20], [1, 27]],
            dtype=tf.int32,
        )  # expected non filtered idx as noted above

        non_inf_expected_output = tf.convert_to_tensor(
            [8.222099, 7.3534126, 8.432078, 7.4402075, 9.38451,
                6.271159, 8.827531, 5.4402995, 7.3857956, 9.677023],
            dtype=tf.float32,
        ...
        non_inf_idx = tf.cast(
            tf.where(tf.not_equal(
                output, tf.constant(-float("inf"), dtype=tf.float32))),
            dtype=tf.int32,
        )
