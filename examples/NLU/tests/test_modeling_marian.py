...
class MarianModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_marian_inputs_dict(
    ...
    def create_and_check_decoder_model_past_large_inputs(self, config, inputs_dict):
        model = MarianModel(config=config).get_decoder().to(
            torch_device).eval()
    ...
    def check_encoder_decoder_model_standalone(self, config, inputs_dict):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            encoder = MarianEncoder.from_pretrained(
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            decoder = MarianDecoder.from_pretrained(
                tmpdirname).to(torch_device)
...
class MarianModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        MarianModel, MarianMTModel) if is_torch_available() else ()
    all_generative_model_classes = (
        MarianMTModel,) if is_torch_available() else ()
...
class MarianIntegrationTest(unittest.TestCase):
    ...
    @cached_property
    def model(self):
        model: MarianMTModel = AutoModelWithLMHead.from_pretrained(
...
class TestMarian_EN_DE_More(MarianIntegrationTest):
    ...
    def test_forward(self):
        ...
        model_inputs = self.tokenizer(
            src, return_tensors="pt").to(torch_device)
    ...
    def test_unk_support(self):
        t = self.tokenizer
        ids = t(["||"], return_tensors="pt").to(
            torch_device).input_ids[0].tolist()
        expected = [t.unk_token_id, t.unk_token_id, t.eos_token_id]
        self.assertEqual(expected, ids)

    def test_pad_not_split(self):
        input_ids_w_pad = self.tokenizer(
            ["I am a small frog <pad>"], return_tensors="pt").input_ids[0].tolist()
        expected_w_pad = [38, 121, 14, 697, 38848,
                          self.tokenizer.pad_token_id, 0]  # pad
...
class TestConversionUtils(unittest.TestCase):
    def test_renaming_multilingual(self):
        ...
        expected = ["opus-mt-ZH-fi", "opus-mt-cmn_cn-fi",
                    "opus-mt-en-de", "opus-mt-en-de"]
        self.assertListEqual(
            expected, [convert_opus_name_to_hf_name(x) for x in old_names])

    def test_undoing_renaming(self):
        hf_names = ["opus-mt-ZH-fi", "opus-mt-cmn_cn-fi",
                    "opus-mt-en-de", "opus-mt-en-de"]
        converted_opus_names = [
            convert_hf_name_to_opus_name(x) for x in hf_names]
        expected_opus_names = [
...
class MarianStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    all_generative_model_classes = (
        MarianForCausalLM,) if is_torch_available() else ()
    test_pruning = False
    is_encoder_decoder = False
    ...
    def setUp(
        ...
        self.model_tester = MarianStandaloneDecoderModelTester(
