...
class MBartModelTester:
    ...
    def prepare_config_and_inputs(self):
        ...
        inputs_dict = prepare_mbart_inputs_dict(
...
class MBartEnroIntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    expected_src_tokens = [8274, 127873, 25916, 7, 8622,
                           2071, 438, 67485, 53, 187895, 23, 51712, 2, 250004]
...
class MBartCC25IntegrationTest(AbstractSeq2SeqIntegrationTest):
    ...
    tgt_text = [
        "Şeful ONU declară că nu există o soluţie militară în Siria", "to be padded"]
    ...
    def test_fill_mask(self):
        inputs = self.tokenizer(
            ["One of the best <mask> I ever read!"], return_tensors="pt").to(torch_device)
...
class MBartStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        MBartDecoder, MBartForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (
