...
class DebertaModelTest(ModelTesterMixin, unittest.TestCase):
    ...
    class DebertaModelTester(object):
        ...
        def create_and_check_deberta_model(
            ...
            sequence_output = model(
                input_ids, attention_mask=input_mask, token_type_ids=token_type_ids)[0]
            sequence_output = model(
                input_ids, token_type_ids=token_type_ids)[0]
    ...
    def test_for_masked_lm(self):
        config_and_inputs = self.model_tester.prepare_config_and_inputs()
        self.model_tester.create_and_check_deberta_for_masked_lm(
            *config_and_inputs)
...
class DebertaModelIntegrationTest(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        expected_slice = torch.tensor(
            [[[-0.5986, -0.8055, -0.8462], [1.4484, -
                                            0.9348, -0.8059], [0.3123, 0.0032, -1.4131]]]
        )
        self.assertTrue(torch.allclose(
            output[:, 1:4, 1:4], expected_slice, atol=1e-4), f"{output[:, 1:4, 1:4]}")
