...
class BartHeadTests(unittest.TestCase):
    ...
    def test_lm_forward(self):
        ...
        lm_labels = ids_tensor(
            [batch_size, input_ids.shape[1]], self.vocab_size).to(torch_device)
    ...
    def test_tokenization(self):
        ...
        # need leading spaces for equality
...
class FastIntegrationTests(unittest.TestCase):
    ...
    def test_encoder_equiv(self):
        ...
        expected = [[-0.0828, -0.0251, -0.0674],
                    [0.1277, 0.3311, -0.0255], [0.2613, -0.0840, -0.2763]]
...
class BartModelIntegrationTests(unittest.TestCase):
    ...
    def test_inference_no_head(self):
        ...
        self.assertTrue(torch.allclose(
            output[:, :3, :3], expected_slice, atol=1e-3))
    ...
    def test_large_mask_filling(self):
        ...
        expected_results = [" bathroom", " gym",
                            " wrong", " movies", " hospital"]
    ...
    def test_mnli_inference(self):
        ...
        expected_slice = torch.Tensor(
            [[0.1907, 1.4342, -1.0289]]).to(torch_device)
    ...
    def test_xsum_config_generation_params(self):
        ...
        expected_params = dict(num_beams=6, do_sample=False,
                               early_stopping=True, length_penalty=1.0)
        config_params = {k: getattr(config, k, "MISSING")
...
class BartStandaloneDecoderModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    all_model_classes = (
        BartDecoder, BartForCausalLM) if is_torch_available() else ()
    all_generative_model_classes = (
