...
class TFLxmertModelTester(object):
    def __init__(
        ...
        self.scope = scope
        self.num_hidden_layers = {"vision": r_layers,
                                  "cross_encoder": x_layers, "language": l_layers}
            ...
            obj_labels["feat"] = (
                ids_tensor(
                    [self.batch_size, self.num_visual_features,
                     self.visual_feat_dim], self.num_visual_features
                ),
                ids_tensor([self.batch_size, self.num_visual_features],
    ...
    def create_and_check_lxmert_model(
        ...
        result = model(input_ids, visual_feats,
                       bounding_boxes, return_dict=False)
        result = model(input_ids, visual_feats,
                       bounding_boxes, return_dict=True)

        self.parent.assertEqual(result.language_output.shape,
                                (self.batch_size, self.seq_length, self.hidden_size))
        self.parent.assertEqual(
            result.vision_output.shape, (self.batch_size,
                                         self.num_visual_features, self.hidden_size)
        )
        self.parent.assertEqual(
...
class TFLxmertModelTest(TFModelTesterMixin, unittest.TestCase):
    ...
    def test_attention_outputs(self):
        ...
        encoder_key_length = (
            self.model_tester.key_length if hasattr(
                self.model_tester, "key_length") else encoder_seq_length
        )
        ...
        for model_class in self.all_model_classes:
            ...
            self.assertEqual(len(cross_encoder_attentions),
                             self.model_tester.num_hidden_layers["cross_encoder"])

            attentions = [language_attentions,
                          vision_attentions, cross_encoder_attentions]
            attention_shapes = [
            ...
            for attention, attention_shape in zip(attentions, attention_shapes):
                self.assertListEqual(
                    list(attention[0].shape[-3:]), attention_shape)
            out_len = len(outputs)
    ...
    def test_hidden_states_output(self):
        ...
        def check_hidden_states_output(config, inputs_dict, model_class):
            ...
            self.assertEqual(len(language_hidden_states),
                ...
                for key, value in iterable.items():
                    if isinstance(value, dict):
                        ...
                        if isinstance(value, (list, tuple)):
                            ...
                            return_dict[key] = torch.from_numpy(
                                value.numpy()).to(torch_type(key))
                return return_dict
    ...
    def test_compile_tf_model(self):
        ...
        for model_class in self.all_model_classes:
            ...
            input_ids = tf.keras.Input(
                batch_shape=(self.model_tester.batch_size,
            ...
            visual_pos = tf.keras.Input(
                batch_shape=(self.model_tester.batch_size,
                             self.model_tester.num_visual_features, 4),
                name="visual_pos",
                dtype="int32",
            )
            ...
            with tempfile.TemporaryDirectory() as tmpdirname:
                outputs = model(self._prepare_for_class(
                    inputs_dict, model_class))  # build the model
                model.save_pretrained(tmpdirname)
                model = model_class.from_pretrained(tmpdirname)
