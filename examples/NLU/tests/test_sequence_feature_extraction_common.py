...
class SequenceFeatureExtractionTestMixin(FeatureExtractionSavingTestMixin):
    ...
    def test_batch_feature(self):
        ...
        self.assertTrue(all(len(x) == len(y) for x, y in zip(
            speech_inputs, processed_features[input_name])))

        speech_inputs = self.feat_extract_tester.prepare_inputs_for_common(
            equal_length=True)
        processed_features = BatchFeature(
            {input_name: speech_inputs}, tensor_type="np")
    ...
    def test_batch_feature_pt(self):
        ...
        processed_features = BatchFeature(
            {input_name: speech_inputs}, tensor_type="pt")
    ...
    def test_batch_feature_tf(self):
        ...
        processed_features = BatchFeature(
            {input_name: speech_inputs}, tensor_type="tf")
    ...
    def _check_padding(self, numpify=False):
        ...
        input_1 = feat_extract.pad(
            processed_features, padding=False)[input_name]
        input_2 = feat_extract.pad(
            processed_features, padding="longest")[input_name]
        input_3 = feat_extract.pad(processed_features, padding="max_length", max_length=len(speech_inputs[-1]))[
            input_name
        ]
        input_4 = feat_extract.pad(
            processed_features, padding="longest", return_tensors="np")[input_name]
        ...
        if feature_size > 1:
            self.assertTrue(input_4.shape[2] ==
                            input_5.shape[2] == feature_size)

        # test padding for `pad_to_multiple_of` for List[int] + numpy
        input_6 = feat_extract.pad(
            processed_features, pad_to_multiple_of=10)[input_name]
        input_7 = feat_extract.pad(
            processed_features, padding="longest", pad_to_multiple_of=10)[input_name]
        ...
        padding_vector_sum = (np.ones(
            self.feat_extract_tester.feature_size) * feat_extract.padding_value).sum()
        self.assertTrue(
            abs(np.asarray(input_2[0])[pad_min_length:].sum(
        ...
        self.assertTrue(
            abs(
                ...
                - padding_vector_sum *
                (pad_max_length - pad_min_length - pad_diff)
            )
            < 1e-3
        )
        self.assertTrue(
            abs(
                ...
                - padding_vector_sum *
                (pad_max_length - pad_min_length - 2 * pad_diff)
            )
            < 1e-3
        )
        self.assertTrue(
        ...
        self.assertTrue(
            abs(input_9[0, pad_min_length:].sum() - padding_vector_sum *
                (expected_mult_pad_length - pad_min_length))
            < 1e-3
        )
    ...
    def test_padding_accepts_tensors_pt(self):
        ...
        input_pt = feat_extract.pad(
            processed_features, padding="longest", return_tensors="pt")[input_name]

        self.assertTrue(
            abs(input_np.astype(np.float32).sum() - input_pt.numpy().sum()) < 1e-2)
    ...
    def test_padding_accepts_tensors_tf(self):
        ...
        input_tf = feat_extract.pad(
            processed_features, padding="longest", return_tensors="tf")[input_name]

        self.assertTrue(
            abs(input_np.astype(np.float32).sum() - input_tf.numpy().sum()) < 1e-2)
    ...
    def test_attention_mask(self):
        ...
        processed = feat_extract.pad(
            processed, padding="longest", return_tensors="np")
        self.assertIn("attention_mask", processed)
        self.assertListEqual(list(processed.attention_mask.shape), list(
            processed[input_name].shape[:2]))
        self.assertListEqual(
            processed.attention_mask.sum(-1).tolist(), input_lenghts)
