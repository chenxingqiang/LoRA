...
class GPT2ModelTester:
    ...
    def create_and_check_double_lm_head_model(
        ...
        self.parent.assertEqual(result.mc_logits.shape,
...
class GPT2ModelTest(ModelTesterMixin, GenerationTesterMixin, unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config_tester = ConfigTester(
            self, config_class=GPT2Config, n_embd=37)
    ...
    def test_batch_generation(self):
        ...
        num_paddings = inputs_non_padded.shape[-1] - \
        ...
        padded_sentence = tokenizer.decode(
            output_padded[0], skip_special_tokens=True)
...
class GPT2ModelLanguageGenerationTest(unittest.TestCase):
    ...
    def test_lm_generate_gpt2(self):
        for checkpointing in [True, False]:
            model = GPT2LMHeadModel.from_pretrained(
                "gpt2", gradient_checkpointing=checkpointing)
            model.to(torch_device)
            input_ids = torch.tensor(
                [[464, 3290]], dtype=torch.long, device=torch_device)  # The dog
            expected_output_ids = [
                464,
                3290,
                373,
    ...
    def test_gpt2_sample(self):
        ...
        output_seq_strs = tokenizer.batch_decode(
        ...
        self.assertTrue(
            all([output_seq_strs[idx] != output_seq_tt_strs[idx]
    ...
    def test_gpt2_sample_max_time(self):
        ...
        model.generate(input_ids, do_sample=True,
                       max_time=MAX_TIME, max_length=256)
        ...
        start = datetime.datetime.now()
        model.generate(input_ids, do_sample=False, num_beams=2,
                       max_time=MAX_TIME, max_length=256)
        duration = datetime.datetime.now() - start
        self.assertGreater(duration, datetime.timedelta(seconds=MAX_TIME))
        self.assertLess(duration, datetime.timedelta(seconds=1.5 * MAX_TIME))
        ...
        start = datetime.datetime.now()
        model.generate(input_ids, do_sample=False,
                       max_time=None, max_length=256)
        duration = datetime.datetime.now() - start
        self.assertGreater(
            duration, datetime.timedelta(seconds=1.5 * MAX_TIME))
