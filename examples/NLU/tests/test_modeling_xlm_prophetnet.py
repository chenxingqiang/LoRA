...
class XLMProphetNetModelIntegrationTest(unittest.TestCase):
    ...
    def test_pretrained_checkpoint_hidden_states(self):
        ...
        expected_slice = torch.tensor(
            [[[-6.6042, -8.3838, 12.4717], [-6.4426, -8.1994,
                                            12.4542], [-6.0851, -7.8209, 12.9493]]]
        ).to(torch_device)
        self.assertTrue(torch.allclose(
            output_predited_logis[:, :3, :3], expected_slice, atol=1e-4))

        # encoder outputs
        encoder_outputs = model.prophetnet.encoder(encoder_ids)[0]
        expected_encoder_outputs_slice = torch.tensor(
            [[[-1.4260, -0.7628, 0.8453], [-1.4719, -
                                           0.1391, 0.7807], [-1.7678, 0.0114, 0.4646]]]
        ).to(torch_device)
    ...
    def test_ntg_hidden_states(self):
        ...
        expected_slice = torch.tensor(
            [[[-8.8815, -9.2996, -4.4506], [-6.7202, -7.8944, -0.9402],
    ...
    def test_xprophetnet_ntg_inference(self):
        ...
        generated_titles = [tokenizer.decode(
