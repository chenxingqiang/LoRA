...
def main():
    ...
    torch.use_deterministic_algorithms(
        training_args.use_deterministic_algorithms)
    ...
    if model_args.apply_adapter:
        if model_args.adapter_path is not None:
            adapter_state_dict = torch.load(os.path.join(
                model_args.adapter_path, 'pytorch_adapter.bin'))
            head_state_dict = torch.load(os.path.join(
                model_args.adapter_path, 'pytorch_model_head.bin'))
            added_state_dict = {}
            ...
            for k, v in head_state_dict.items():
                ...
                added_state_dict[new_k] = v
            logger.info(
                f"Apply adapter state dict from {model_args.adapter_path}.")
            ...
            assert len(unexpected_keys) == 0, 'Unexpected keys ' + \
                str(unexpected_keys)
