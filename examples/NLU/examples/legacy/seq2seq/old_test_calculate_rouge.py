...
def test_newline_cnn_improvement():
    ...
    score_no_sep = calculate_rouge(
        PRED, TGT, newline_sep=False, rouge_keys=[k])[k]
...
def test_pegasus_newline():
    ...
    prev_score = calculate_rouge(pred, tgt, rouge_keys=[
                                 "rougeLsum"], newline_sep=False)["rougeLsum"]
    new_score = calculate_rouge(pred, tgt, rouge_keys=[
                                "rougeLsum"])["rougeLsum"]
...
def test_rouge_cli():
    ...
    metrics = calculate_rouge_path(data_dir.joinpath(
        "test.source"), data_dir.joinpath("test.target"))
