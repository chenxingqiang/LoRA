...
class TestAll(TestCasePlus):
    ...
    def test_pack_dataset(self):
        ...
        save_dir = Path(make_test_data_dir(
            tmp_dir=self.get_auto_remove_tmp_dir()))
    ...
    def test_sortish_sampler_reduces_padding(self):
        ...
        assert sum(count_pad_tokens(sortish_dl)) < sum(
            count_pad_tokens(naive_dl))
    ...
    def test_distributed_sortish_sampler_splits_indices_between_procs(self):
        ...
        ids1 = set(DistributedSortishSampler(
            ds, 256, num_replicas=2, rank=0, add_extra_examples=False))
        ids2 = set(DistributedSortishSampler(
