...
def do_nms(boxes, scores, image_shape, score_thresh, nms_thresh, mind, maxd):
    ...
    idxs = torch.arange(num_objs).to(boxes.device) * \
        num_bbox_reg_classes + max_classes
...
class Matcher(object):
    ...
    def set_low_quality_matches_(self, match_labels, match_quality_matrix):
        ...
        if of_quality_inds.dim() == 0:
            (_, pred_inds_with_highest_quality) = of_quality_inds.unsqueeze(
                0).nonzero().unbind(1)
        else:
...
class RPNOutputs(object):
    ...
    def predict_proposals(self):
        ...
        for anchors_i, pred_anchor_deltas_i in zip(anchors, self.pred_anchor_deltas):
            ...
            pred_anchor_deltas_i = pred_anchor_deltas_i.view(
    ...
    def make_stage(
        ...
        **kwargs,
        ...
        if first_stride is not None:
            ...
            kwargs["stride_per_block"] = [
                first_stride] + [1] * (num_blocks - 1)
        blocks = []
...
class ROIPooler(nn.Module):
    ...
    def __init__(
        ...
        assert len(output_size) == 2 and isinstance(
            output_size[0], int) and isinstance(output_size[1], int)
        if len(scales) > 1:
...
class ROIOutputs(object):
    ...
    def _predict_boxes(self, proposals, box_deltas, preds_per_image):
        ...
        proposals = torch.cat(
            proposals, dim=0).unsqueeze(-2).expand(num_pred, K, B)
    ...
    def inference(
        ...
        boxes_all = self._predict_boxes(
            pred_boxes, box_deltas, preds_per_image)
        obj_scores_all = self._predict_objs(
            obj_logits, preds_per_image)  # list of length N
        attr_probs_all, attrs_all = self._predict_attrs(
            attr_logits, preds_per_image)
        features = features.split(preds_per_image, dim=0)

        # fun for each image too, also I can experiment and do multiple images
        final_results = []
        zipped = zip(boxes_all, obj_scores_all,
                     attr_probs_all, attrs_all, sizes)
...
class AnchorGenerator(nn.Module):
    ...
    def forward(self, features):
        ...
        anchors_over_all_feature_maps = torch.stack(
...
class RPNHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        assert len(set(num_cell_anchors)
                   ) == 1, "Each level must have the same number of cell anchors"
        ...
        self.conv = nn.Conv2d(in_channels, hid_channels,
...
class RPN(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.box2box_transform = Box2BoxTransform(
            weights=cfg.RPN.BBOX_REG_WEIGHTS)
        self.anchor_matcher = Matcher(
            cfg.RPN.IOU_THRESHOLDS,
            cfg.RPN.IOU_LABELS,
            allow_low_quality_matches=True,
        )
        self.rpn_head = RPNHead(cfg, [input_shape[f]
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        if use_attr:
            ...
            self.fc_attr = nn.Linear(
                input_size + input_size // 8, input_size // 4)
...
class GeneralizedRCNN(nn.Module):
    ...
    def inference(
        ...
        **kwargs,
        ...
        class_probs = pad_list_tensors(
            class_probs, preds_per_image, **subset_kwargs)
        attrs = pad_list_tensors(attrs, preds_per_image, **subset_kwargs)
        attr_probs = pad_list_tensors(
            attr_probs, preds_per_image, **subset_kwargs)
        roi_features = pad_list_tensors(
            roi_features, preds_per_image, **subset_kwargs)
        subset_kwargs["padding"] = None
        preds_per_image = pad_list_tensors(
            preds_per_image, None, **subset_kwargs)
