...
def embed(documents: dict, ctx_encoder: DPRContextEncoder, ctx_tokenizer: DPRContextEncoderTokenizerFast) -> dict:
    ...
    embeddings = ctx_encoder(input_ids.to(
        device=device), return_dict=True).pooler_output
...
def main(
    ...
    new_features = Features(
        {"text": Value("string"), "title": Value("string"),
    ...
    passages_path = os.path.join(
        rag_example_args.output_dir, "my_knowledge_dataset")
