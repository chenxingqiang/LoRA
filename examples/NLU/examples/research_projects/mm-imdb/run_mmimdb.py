    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        if preds is None:
            ...
            preds = np.append(preds, torch.sigmoid(
                logits).detach().cpu().numpy() > 0.5, axis=0)
...
def load_examples(args, tokenizer, evaluate=False):
    path = os.path.join(
        args.data_dir, "dev.jsonl" if evaluate else "train.jsonl")
...
def main():
    ...
    if args.do_train:
        ...
        label_weights = (
            torch.tensor(label_frequences, device=args.device,
                         dtype=torch.float) / len(train_dataset)
        ) ** -1
