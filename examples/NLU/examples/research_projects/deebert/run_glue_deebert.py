    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if eval_highway:
            ...
            if args.early_exit_entropy >= 0:
                ...
                np.save(save_fname, np.array(
                    [exit_layer_counter, eval_time, actual_cost / full_cost, print_result]))
...
def main():
    ...
    parser.add_argument("--early_exit_entropy", default=-1,
                        type=float, help="Entropy threshold for early exit.")
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            if args.model_type == "bert":
                model.bert.encoder.set_early_exit_entropy(
                    args.early_exit_entropy)
            elif args.model_type == "roberta":
                model.roberta.encoder.set_early_exit_entropy(
                    args.early_exit_entropy)
            else:
                raise NotImplementedError()
            ...
            result = evaluate(args, model, tokenizer,
                              prefix=prefix, eval_highway=args.eval_highway)
            print_result = get_wanted_result(result)
            ...
            if args.eval_each_highway:
                ...
                save_fname = args.plot_data_dir + "/" + \
                    args.model_name_or_path[2:] + "/each_layer.npy"
                if not os.path.exists(os.path.dirname(save_fname)):
                    os.makedirs(os.path.dirname(save_fname))
