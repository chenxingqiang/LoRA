        ...
        if compute_entropy:
            for layer, attn in enumerate(all_attentions):
                masked_entropy = entropy(
                    attn.detach()) * inputs["attention_mask"].float().unsqueeze(1)
...
def mask_heads(args, model, eval_dataloader):
    ...
    preds = np.argmax(
        preds, axis=1) if args.output_mode == "classification" else np.squeeze(preds)
    original_score = glue_compute_metrics(
        args.task_name, preds, labels)[args.metric_name]
...
def prune_heads(args, model, eval_dataloader, head_mask):
    ...
    logger.info("Pruning: speed ratio (new timing / original timing): %f percents",
...
def main():
    ...
    if args.data_subset > 0:
        eval_dataset = Subset(eval_dataset, list(
            range(min(args.data_subset, len(eval_dataset)))))
