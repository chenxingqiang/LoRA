...
class SummarizationDataProcessingTest(unittest.TestCase):
    ...
    def test_fit_to_block_sequence_too_small(self):
        ...
        self.assertEqual(truncate_or_pad(
            sequence, self.block_size, 0), expected_output)
    ...
    def test_build_mask_no_padding(self):
        ...
        np.testing.assert_array_equal(build_mask(
            sequence, 0).numpy(), expected.numpy())

    def test_build_mask(self):
        sequence = torch.tensor([1, 2, 3, 4, 23, 23, 23])
        expected = torch.tensor([1, 1, 1, 1, 0, 0, 0])
        np.testing.assert_array_equal(build_mask(
            sequence, 23).numpy(), expected.numpy())

    def test_build_mask_with_padding_equal_to_one(self):
        sequence = torch.tensor([8, 2, 3, 4, 1, 1, 1])
        expected = torch.tensor([1, 1, 1, 1, 0, 0, 0])
        np.testing.assert_array_equal(build_mask(
            sequence, 1).numpy(), expected.numpy())
