...
def convert_bertabs_checkpoints(path_to_checkpoints, dump_path):
    ...
    # loaded weights.
    assert torch.max(
        torch.abs(original.generator[0].weight - new_model.generator[0].weight)) == 0
    ...
    output_original_model = original(
        src, tgt, segs, clss, mask_src, mask_tgt, mask_cls)[0]
    ...
    maximum_absolute_difference = torch.max(
        torch.abs(output_converted_generator - output_original_generator)).item()
    print("Maximum absolute difference beween weights: {:.2f}".format(
        maximum_absolute_difference))
