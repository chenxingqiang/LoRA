    ...
    for name, param in model.named_parameters():
        if "mask_scores" in name:
            if mode == "l1":
                ...
                regu += torch.sigmoid(param - 2 / 3 *
                                      np.log(0.1 / 1.1)).sum() / param.numel()
            else:
            ...
            if teacher is not None:
                ...
                loss_start = (
                    F.kl_div(
                        ...
                        target=F.softmax(start_logits_tea /
                ...
                loss_end = (
                    F.kl_div(
                        ...
                        target=F.softmax(end_logits_tea /
                                         args.temperature, dim=-1),
                        reduction="batchmean",
                    )
                    * (args.temperature ** 2)
                    ...
                    for name, param in model.named_parameters():
                        ...
                        if args.regularization is not None and "mask_scores" in name:
                            if args.regularization == "l1":
                                perc = (torch.sigmoid(param) >
                                        threshold).sum().item() / param.numel()
                            elif args.regularization == "l0":
                                perc = (torch.sigmoid(
                                    param - 2 / 3 * np.log(0.1 / 1.1))).sum().item() / param.numel()
                            tb_writer.add_scalar(
                        ...
                        if (teacher is not None) and (args.regularization is not None):
                            ...
                            tb_writer.add_scalar(
                                ...
                                (loss.item() - args.alpha_distil *
                                 loss_logits.item()) / args.alpha_ce,
                                global_step,
                            )
                        else:
                            tb_writer.add_scalar(
...
def main():
    ...
    parser.add_argument("--global_topk", action="store_true",
                        help="Global TopK on the Scores.")
    parser.add_argument(
    ...
    output_eval_file = os.path.join(
        args.output_dir, predict_file, "eval_results.txt")
