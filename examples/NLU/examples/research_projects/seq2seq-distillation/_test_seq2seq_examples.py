...
SUMMARIES = ["A very interesting story about what I ate for lunch.",
...
class TestSummarizationDistiller(TestCasePlus):
    ...
    def test_hub_configs(self):
        ...
        org = "sshleifer"
        model_ids = [
            x.modelId for x in model_list if x.modelId.startswith(org)]
        allowed_to_be_broken = [
            "sshleifer/blenderbot-3B", "sshleifer/blenderbot-90M"]
        failures = []
        for m in model_ids:
    ...
    def test_distill_no_teacher(self):
        updates = dict(student_encoder_layers=2,
                       student_decoder_layers=1, no_teacher=True)
    ...
    def test_distill_checkpointing_with_teacher(self):
        ...
        examples = lmap(str.strip, Path(model.hparams.data_dir).joinpath(
            "test.source").open().readlines())
    ...
    def _test_distiller_cli(self, updates, check_contents=True):
        ...
        args_d.update(data_dir=tmp_dir, output_dir=output_dir,
                      **default_updates)
...
class TestTheRest(TestCasePlus):
    ...
    def test_finetune(self, model):
        args_d: dict = CHEAP_ARGS.copy()
        task = "translation" if model in [
            MBART_TINY, MARIAN_TINY, FSMT_TINY] else "summarization"
    ...
    def test_finetune_extra_model_args(self):
        ...
        for p in extra_model_params:
            assert getattr(
                model.config, p) == 0.5, f"failed to override the model config for param {p}"
        ...
        with pytest.raises(Exception) as excinfo:
            model = main(args)
        assert str(
            excinfo.value) == f"model config doesn't have a `{unsupported_param}` attribute"
