...
class OnnxConverterArgumentParser(ArgumentParser):
    ...
    def __init__(self):
        ...
        self.add_argument("--tokenizer", type=str,
        ...
        self.add_argument("--opset", type=int, default=11,
...
def infer_shapes(nlp: Pipeline, framework: str) -> Tuple[List[str], List[str], Dict, BatchEncoding]:
    ...
    def build_shape_dict(name: str, tensor, is_input: bool, seq_len: int):
        if isinstance(tensor, (tuple, list)):
            ...
            axes = {[axis for axis, numel in enumerate(
                tensor.shape) if numel == 1][0]: "batch"}
            if is_input:
                ...
                seq_axes = [dim for dim, shape in enumerate(
                    tensor.shape) if shape == seq_len]
    ...
    input_dynamic_axes = {k: build_shape_dict(
        k, v, True, seq_len) for k, v in tokens.items()}
    ...
    output_dynamic_axes = {k: build_shape_dict(
        k, v, False, seq_len) for k, v in zip(output_names, outputs_flat)}
...
def convert_tensorflow(nlp: Pipeline, opset: int, output: Path):
    ...
    try:
        ...
        onnx_model = convert_keras(
            nlp.model, nlp.model.name, target_opset=opset)
...
def optimize(onnx_model_path: Path) -> Path:
    ...
    opt_model_path = generate_identified_filename(
        onnx_model_path, "-optimized")
...
def verify(path: Path):
    ...
    try:
        ...
        _ = InferenceSession(path.as_posix(), onnx_options,
