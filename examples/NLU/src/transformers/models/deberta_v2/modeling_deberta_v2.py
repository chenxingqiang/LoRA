...
class ConvLayer(nn.Module):
    ...
    def forward(self, hidden_states, residual_states, input_mask):
        out = self.conv(hidden_states.permute(
            0, 2, 1).contiguous()).permute(0, 2, 1).contiguous()
    ...
    def __init__(self, config):
        ...
        if self.relative_attention:
            ...
            self.rel_embeddings = nn.Embedding(
                pos_ebd_size, config.hidden_size)

        self.norm_rel_ebd = [x.strip() for x in getattr(
            config, "norm_rel_ebd", "none").lower().split("|")]

        if "layer_norm" in self.norm_rel_ebd:
            self.LayerNorm = LayerNorm(
                config.hidden_size, config.layer_norm_eps, elementwise_affine=True)

        self.conv = ConvLayer(config) if getattr(
            config, "conv_kernel_size", 0) > 0 else None
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if query_states is not None:
                ...
                if isinstance(hidden_states, Sequence):
                    next_kv = hidden_states[i + 1] if i + \
                        1 < len(self.layer) else None
            else:
...
def make_log_bucket_position(relative_pos, bucket_size, max_position):
    ...
    mid = bucket_size // 2
    abs_pos = np.where((relative_pos < mid) & (
        relative_pos > -mid), mid - 1, np.abs(relative_pos))
    log_pos = np.ceil(np.log(abs_pos / mid) /
                      np.log((max_position - 1) / mid) * (mid - 1)) + mid
    bucket_pos = np.where(abs_pos <= mid, relative_pos,
                          log_pos * sign).astype(np.int)
    return bucket_pos
...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def forward(
        ...
        attention_scores = torch.bmm(
            query_layer, key_layer.transpose(-1, -2)) / scale
        if self.relative_attention:
    ...
    def manually_gather(self, input, index):
        ...
        index = index + torch.arange(start=0, end=index.size(
            0)*input.shape[-1], step=input.shape[-1], device=index.device).view(-1, 1)
    ...
    def disentangled_attention_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        rel_embeddings = rel_embeddings[self.pos_ebd_size -
                                        att_span: self.pos_ebd_size + att_span, :].unsqueeze(0)
        if self.share_att_key:
            ...
            if "p2c" in self.pos_att_type or "p2p" in self.pos_att_type:
                pos_query_layer = self.transpose_for_scores(
                    self.pos_query_proj(
                        rel_embeddings), self.num_attention_heads
                ).repeat(
                    query_layer.size(0) // self.num_attention_heads, 1, 1
                )  # .split(self.all_head_size, dim=-1)
        ...
        if "p2p" in self.pos_att_type:
            ...
            p2p_att = torch.gather(
                ...
                index=c2p_pos.expand(
                    [query_layer.size(0), query_layer.size(
                        1), query_layer.size(2), relative_pos.size(-1)]
                ),
            )
            score += p2p_att
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, nn.Linear):
            ...
            if hasattr(module.query_proj, 'lora_A'):
                module.query_proj.lora_A.data.normal_(
                    mean=0.0, std=self.config.initializer_range)
            if hasattr(module.value_proj, 'lora_A'):
                module.value_proj.lora_A.data.normal_(
                    mean=0.0, std=self.config.initializer_range)
...
class DebertaV2ForCutoffSequenceClassification(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        if self.masking_prob > 0:
            random_indices = torch.bernoulli(torch.full(
                input_ids.shape, self.masking_prob, device=input_ids.device)).bool()
            masking_indces = (input_ids != self.cls_token_id) & (
                input_ids != self.sep_token_id) & random_indices & attention_mask.bool()
        ...
        if labels is not None:
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                if label_index.size(0) > 0:
                    p1_logits = torch.index_select(
                        p1_logits, 0, label_index.view(-1))
                    p2_logits = torch.index_select(
                        p2_logits, 0, label_index.view(-1))
                    m_logits = torch.index_select(
                        m_logits, 0, label_index.view(-1))
                    labels = torch.index_select(
                        labels, 0, label_index.view(-1))
                    loss_fct = CrossEntropyLoss()
                    p1_loss = loss_fct(
                        p1_logits.view(-1, self.num_labels).float(), labels.view(-1))
                    p2_loss = loss_fct(
                        p2_logits.view(-1, self.num_labels).float(), labels.view(-1))
                    p1_m_loss = F.kl_div(F.log_softmax(
                        p1_logits, dim=-1), F.softmax(m_logits, dim=-1), reduction='mean')
                    p2_m_loss = F.kl_div(F.log_softmax(
                        p2_logits, dim=-1), F.softmax(m_logits, dim=-1), reduction='mean')
                else:
                ...
                p2_m_loss = F.kl_div(F.log_softmax(
                    p2_logits, dim=-1), F.softmax(m_logits, dim=-1), reduction='mean')
            loss = p1_loss + p2_loss + 0.5 * \
                self.reg_loss_wgt * (p1_m_loss + p2_m_loss)
        if not return_dict:
            return (loss,)
        else:
        ...
        if labels is not None:
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                if label_index.size(0) > 0:
                    ...
                    p2_loss = loss_fct(
                        p2_logits.view(-1, self.num_labels).float(), labels.view(-1))
                    p12_loss = F.kl_div(F.log_softmax(
                        p1_logits, dim=-1), F.softmax(p2_logits, dim=-1), reduction='mean')
                    p21_loss = F.kl_div(F.log_softmax(
                        p2_logits, dim=-1), F.softmax(p1_logits, dim=-1), reduction='mean')
                else:
                    p1_loss = torch.tensor(0).to(p1_logits)
                    p2_loss = torch.tensor(0).to(p2_logits)
                ...
                p21_loss = F.kl_div(F.log_softmax(
                    p2_logits, dim=-1), F.softmax(p1_logits, dim=-1), reduction='mean')
            loss = p1_loss + p2_loss + 0.5 * \
                self.reg_loss_wgt * (p12_loss + p21_loss)
        if not return_dict:
            return (loss,)
        else:
            return SequenceClassifierOutput(
    ...
    def forward(
        ...
        if labels is not None and self.reg_loss_wgt == 0:
            if self.num_labels == 1:
                ...
                labels = labels.long()
                if label_index.size(0) > 0:
                    labeled_logits = torch.index_select(
                        logits, 0, label_index.view(-1))
                    labels = torch.index_select(
                        labels, 0, label_index.view(-1))
                    loss_fct = CrossEntropyLoss()
                    loss = loss_fct(
            ...
            if labels.dim() == 1 or labels.size(-1) == 1:
                ...
                labels = labels.long()
                if label_index.size(0) > 0:
                    logits_1 = torch.index_select(
                        logits, 0, label_index.view(-1))
                    logits_2 = torch.index_select(
                        logits_2, 0, label_index.view(-1))
                    logits_m = torch.index_select(
                        logits_m, 0, label_index.view(-1))
                    labels = torch.index_select(
                        labels, 0, label_index.view(-1))
                    loss_fct = CrossEntropyLoss()
                    loss_1 = loss_fct(
                        logits_1.view(-1, self.num_labels).float(), labels.view(-1))
                    loss_2 = loss_fct(
                        logits_2.view(-1, self.num_labels).float(), labels.view(-1))
                    kl_loss_1 = F.kl_div(F.log_softmax(
                        logits_1, dim=-1), F.softmax(logits_m, dim=-1), reduction='mean')
                    kl_loss_2 = F.kl_div(F.log_softmax(
                        logits_2, dim=-1), F.softmax(logits_m, dim=-1), reduction='mean')
                else:
                ...
                kl_loss_1 = F.kl_div(F.log_softmax(
                    logits, dim=-1), F.softmax(logits_m, dim=-1), reduction='mean')
                kl_loss_2 = F.kl_div(F.log_softmax(
                    logits_2, dim=-1), F.softmax(logits_m, dim=-1), reduction='mean')
            loss = loss_1 + loss_2 + 0.5 * \
                self.reg_loss_wgt * (kl_loss_1 + kl_loss_2)
