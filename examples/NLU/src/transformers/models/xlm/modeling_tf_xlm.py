...
class TFXLMMultiHeadAttention(tf.keras.layers.Layer):
    ...
    def call(self, input, mask, kv, cache, head_mask, output_attentions, training=False):
        ...
        mask_reshape = (bs, 1, qlen, klen) if len(
            shape_list(mask)) == 3 else (bs, 1, 1, klen)
        ...
        for i in range(self.n_layers):
            self.attentions.append(
                TFXLMMultiHeadAttention(
                    self.n_heads, self.dim, config=config, name="attentions_._{}".format(i))
            )
            self.layer_norm1.append(
                tf.keras.layers.LayerNormalization(
                    epsilon=config.layer_norm_eps, name="layer_norm1_._{}".format(i))
            )
            # if self.is_decoder:
            ...
            self.layer_norm2.append(
                tf.keras.layers.LayerNormalization(
                    epsilon=config.layer_norm_eps, name="layer_norm2_._{}".format(i))
            )
    ...
    def call(
        ...
        **kwargs,
        ...
        tensor = inputs["inputs_embeds"] + \
            tf.gather(self.position_embeddings, inputs["position_ids"])
...
class TFXLMPreTrainedModel(TFPreTrainedModel):
    ...
    def dummy_inputs(self):
        ...
        inputs_list = tf.constant(
            [[7, 6, 0, 0, 1], [1, 2, 3, 0, 0], [0, 0, 0, 4, 5]])
        attns_list = tf.constant(
...
class TFXLMForMultipleChoice(TFXLMPreTrainedModel, TFMultipleChoiceLoss):
    ...
    def call(
        ...
        **kwargs,
        ...
        flat_langs = tf.reshape(
            inputs["langs"], (-1, seq_length)) if inputs["langs"] is not None else None
        flat_inputs_embeds = (
