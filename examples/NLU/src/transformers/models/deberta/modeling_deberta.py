...
class DebertaLayerNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = (hidden_states - mean) / \
            torch.sqrt(variance + self.variance_epsilon)
...
class DebertaSelfOutput(nn.Module):
    def __init__(self, config):
        ...
        self.LayerNorm = DebertaLayerNorm(
...
class DebertaEncoder(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.layer = nn.ModuleList([DebertaLayer(config)
...
class DisentangledSelfAttention(torch.nn.Module):
    ...
    def __init__(self, config):
        ...
        self.in_proj = torch.nn.Linear(
            config.hidden_size, self.all_head_size * 3, bias=False)
        self.q_bias = torch.nn.Parameter(torch.zeros(
            (self.all_head_size), dtype=torch.float))
        self.v_bias = torch.nn.Parameter(torch.zeros(
            (self.all_head_size), dtype=torch.float))
        ...
        if self.talking_head:
            self.head_logits_proj = torch.nn.Linear(
        ...
        if self.relative_attention:
            ...
            if "c2p" in self.pos_att_type or "p2p" in self.pos_att_type:
                self.pos_proj = torch.nn.Linear(
                    config.hidden_size, self.all_head_size, bias=False)
            if "p2c" in self.pos_att_type or "p2p" in self.pos_att_type:
                self.pos_q_proj = torch.nn.Linear(
    ...
    def forward(
        ...
        if query_states is None:
            ...
            qkvw = [torch.cat(
                [ws[i * 3 + k] for i in range(self.num_attention_heads)], dim=0) for k in range(3)]
            qkvb = [None] * 3

            q = linear(qkvw[0], qkvb[0], query_states)
            k, v = [linear(qkvw[i], qkvb[i], hidden_states)
                    for i in range(1, 3)]
            query_layer, key_layer, value_layer = [
                self.transpose_for_scores(x) for x in [q, k, v]]

        query_layer = query_layer + \
            self.transpose_for_scores(self.q_bias[None, None, :])
        value_layer = value_layer + \
            self.transpose_for_scores(self.v_bias[None, None, :])
    ...
    def disentangled_att_bias(self, query_layer, key_layer, relative_pos, rel_embeddings, scale_factor):
        ...
        if "c2p" in self.pos_att_type:
            c2p_att = torch.matmul(
                query_layer, pos_key_layer.transpose(-1, -2))
        ...
        # position->content
        if "p2c" in self.pos_att_type or "p2p" in self.pos_att_type:
            pos_query_layer /= math.sqrt(pos_query_layer.size(-1)
                                         * scale_factor)
        ...
        if "p2c" in self.pos_att_type:
            p2c_att = torch.matmul(
                key_layer, pos_query_layer.transpose(-1, -2))
