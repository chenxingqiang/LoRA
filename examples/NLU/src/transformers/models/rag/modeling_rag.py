...
class RagModel(RagPreTrainedModel):
    def __init__(
        ...
        # or maybe just use a `set_retriever(...)` method
        ...
        if encoder_outputs is None:
            ...
            if has_to_retrieve:
                ...
                doc_scores = torch.bmm(
                    question_encoder_last_hidden_state.unsqueeze(
                        1), retrieved_doc_embeds.transpose(1, 2)
                ).squeeze(1)
            else:
                assert (
...
class RagSequenceForGeneration(RagPreTrainedModel):
        ...
        **model_kwargs
        ...
        for index in range(batch_size):
            ...
            if input_ids is not None:
                new_input_ids = input_ids[index: index +
                                          1].repeat(num_candidates, 1)
                outputs = self(
            ...
            top_cand_inds = (-outputs["loss"]
                             ).topk(num_doc_return_sequences)[1]
    ...
    def get_nll(
        ...
        # shift tokens left
        target = torch.cat(
            [target[:, 1:], target.new(target.shape[0], 1).fill_(
        ...
        doc_logprobs = torch.nn.functional.log_softmax(
    ...
    def _cat_and_pad(tensors, pad_token_id):
        output = (
            tensors[0].new(sum([t.shape[0] for t in tensors]), max(
        ...
        **model_kwargs
        ...
        def extend_enc_output(tensor, num_beams=None):
            ...
            tensor = tensor.expand(
                (batch_size, num_beams, n_docs) + tensor.shape[3:])
