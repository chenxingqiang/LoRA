...
class TFT5LayerNorm(tf.keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.weight = self.add_weight("weight", shape=(
            input_shape[-1],), initializer="ones")
        super().build(input_shape)

    def call(self, hidden_states):
        variance = tf.math.reduce_mean(tf.math.square(
            hidden_states), axis=-1, keepdims=True)
        hidden_states = hidden_states * \
            tf.math.rsqrt(variance + self.variance_epsilon)
...
class TFT5DenseReluDense(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.wo = tf.keras.layers.Dense(
            config.d_model, use_bias=False, name="wo")
...
class TFT5GatedGeluDense(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.wi_0 = tf.keras.layers.Dense(
            config.d_ff, use_bias=False, name="wi_0")
        self.wi_1 = tf.keras.layers.Dense(
            config.d_ff, use_bias=False, name="wi_1")
        self.wo = tf.keras.layers.Dense(
            config.d_model, use_bias=False, name="wo")
        self.dropout = tf.keras.layers.Dropout(config.dropout_rate)
...
class TFT5LayerFF(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        if config.feed_forward_proj == "relu":
            self.DenseReluDense = TFT5DenseReluDense(
                config, name="DenseReluDense")
        elif config.feed_forward_proj == "gated-gelu":
            self.DenseReluDense = TFT5GatedGeluDense(
                config, name="DenseReluDense")
        else:
            raise ValueError(
    ...
    def call(self, hidden_states, training=False):
        ...
        dense_output = self.DenseReluDense(
            normed_hidden_states, training=training)
        hidden_states = hidden_states + \
            self.dropout(dense_output, training=training)
        return hidden_states
...
class TFT5Attention(tf.keras.layers.Layer):
    ...
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        ...
        self.q = tf.keras.layers.Dense(
            self.inner_dim, use_bias=False, name="q")
        self.k = tf.keras.layers.Dense(
            self.inner_dim, use_bias=False, name="k")
        self.v = tf.keras.layers.Dense(
            self.inner_dim, use_bias=False, name="v")
    ...
    def _relative_position_bucket(relative_position, bidirectional=True, num_buckets=32, max_distance=128):
        ...
        relative_buckets += tf.where(is_small,
    ...
    def call(
        ...
        if past_key_value is not None:
            ...
            real_seq_length += shape_list(past_key_value[0])[
                2] if query_length is None else query_length

        key_length = real_seq_length if key_value_states is None else shape_list(
            key_value_states)[1]
...
class TFT5LayerSelfAttention(tf.keras.layers.Layer):
    ...
    def call(
        ...
        hidden_states = hidden_states + \
            self.dropout(attention_output[0], training=training)
...
class TFT5Block(tf.keras.layers.Layer):
    def __init__(self, config, has_relative_attention_bias=False, **kwargs):
        ...
        self.layer.append(TFT5LayerFF(
            config, name="layer_._{}".format(len(self.layer))))
        ...
        **kwargs,
        ...
        if num_dims_attention_mask == 3:
            ...
            if self.is_decoder:
                ...
                causal_mask = tf.cast(
                    causal_mask, dtype=inputs["attention_mask"].dtype)
                ...
                extended_attention_mask = inputs["attention_mask"][:,
                                                                   None, None, :]
...
class TFT5Model(TFT5PreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.encoder = TFT5MainLayer(
            encoder_config, embed_tokens, name="encoder")

        decoder_config = copy.deepcopy(config)
        decoder_config.is_decoder = True
        self.decoder = TFT5MainLayer(
            decoder_config, embed_tokens, name="decoder")
    ...
    def call(
        ...
        **kwargs,
        ...
        if not inputs["return_dict"]:
            past = (inputs["encoder_outputs"], decoder_outputs[1]
                    ) if inputs["use_cache"] else None
            if past is not None:
                decoder_outputs = decoder_outputs[:1] + \
                    (past,) + decoder_outputs[2:]
            return decoder_outputs + inputs["encoder_outputs"]

        past = (inputs["encoder_outputs"].to_tuple(),
                decoder_outputs[1]) if inputs["use_cache"] else None
        ...
        if not inputs["return_dict"]:
            ...
            if past is not None:
                decoder_outputs = decoder_outputs[:1] + \
                    (past,) + decoder_outputs[2:]
            output = (logits,) + \
                decoder_outputs[1:] + inputs["encoder_outputs"]
