...
class TFGPT2MainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config, *inputs, **kwargs):
        ...
        self.h = [TFBlock(config.n_ctx, config, scale=True, name="h_._{}".format(
        ...
        **kwargs,
        ...
        if inputs["attention_mask"] is not None:
            ...
            inputs["attention_mask"] = tf.multiply(
                tf.subtract(
                    one_cst, inputs["attention_mask"]), tf.constant(-10000.0)
            )
        ...
        position_embeds = tf.cast(
            position_embeds, dtype=inputs["inputs_embeds"].dtype)
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["labels"] is not None:
            ...
            loss = self.compute_loss(tf.reshape(
                inputs["labels"], [-1]), tf.reshape(in_logits, [-1, self.num_labels]))
