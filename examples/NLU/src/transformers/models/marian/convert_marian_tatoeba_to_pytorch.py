...
class TatoebaConverter:
    ...
    def __init__(self, save_dir="marian_converted"):
        ...
        released = pd.read_csv(
            "Tatoeba-Challenge/models/released-models.txt", sep="\t", header=None).iloc[:-1]
        released.columns = released_cols
        released["fname"] = released["url_base"].apply(
            lambda x: remove_suffix(remove_prefix(
                x, "https://object.pouta.csc.fi/Tatoeba-Challenge/opus"), ".zip")
        )
        ...
        metadata_renamer = {"src": "src_alpha3", "tgt": "tgt_alpha3",
                            "id": "long_pair", "date": "train_date"}
    ...
    def convert_models(self, tatoeba_ids, dry_run=False):
        ...
        for path in converted_paths:
            long_pair = remove_prefix(
                path.name, "opus-mt-").split("-")  # eg. heb-eng
...
GROUP_MEMBERS = {
    ...
    "bnt": (
        "Bantu languages",
        {"kin", "lin", "lug", "nya", "run", "sna", "swh",
            "toi_Latn", "tso", "umb", "xho", "zul"},
    ),
    ...
    "pqe": (
        ...
        {"fij", "gil", "haw", "mah", "mri", "nau",
            "niu", "rap", "smo", "tah", "ton", "tvl"},
    ),
    "roa": (
        "Romance languages",
        {
...
if __name__ == "__main__":
    ...
    parser.add_argument("-save_dir", "--save_dir",
                        default="marian_converted", help="where to save converted models")
