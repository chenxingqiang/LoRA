...
class ConvBertPreTrainedModel(PreTrainedModel):
    ...
    authorized_unexpected_keys = [
        r"convbert\.embeddings_project\.weight", r"convbert\.embeddings_project\.bias"]
...
class SeparableConv1D(nn.Module):
    ...
    def __init__(self, config, input_filters, output_filters, kernel_size, **kwargs):
        ...
        self.depthwise.weight.data.normal_(
            mean=0.0, std=config.initializer_range)
        self.pointwise.weight.data.normal_(
            mean=0.0, std=config.initializer_range)
...
class ConvBertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        conv_attn_layer = torch.multiply(
        ...
        conv_out_layer = conv_out_layer.transpose(
        ...
        conv_out_layer = torch.reshape(
            conv_out_layer, [-1, self.all_head_size])
        ...
        )[:-2] + (self.head_ratio * self.all_head_size,)
...
class GroupedLinearLayer(nn.Module):
    def __init__(self, input_size, output_size, num_groups):
        ...
        self.weight = nn.Parameter(torch.Tensor(
            self.num_groups, self.group_in_dim, self.group_out_dim))
...
class ConvBertEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([ConvBertLayer(config)
