...
def shift_tokens_right(input_ids: tf.Tensor, pad_token_id: int):
    ...
    language_id_index = (
        tf.reduce_sum(tf.cast(tf.math.not_equal(
            input_ids, pad_token_id), dtype=input_ids.dtype), axis=-1) - 1
    )
...
class TFMBartEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[TFSharedEmbeddings] = None, **kwargs):
        ...
        self.layers = [TFMBartEncoderLayer(
...
class TFMBartDecoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: MBartConfig, embed_tokens: Optional[TFSharedEmbeddings] = None, **kwargs):
        ...
        self.embed_scale = tf.math.sqrt(
            float(config.d_model)) if config.scale_embedding else 1.0
        self.layers = [TFMBartDecoderLayer(
