...
class Conv1dSubsampler(nn.Module):
    ...
    def forward(self, input_features):
        hidden_states = input_features.transpose(
            1, 2).contiguous()  # -> B x (C x D) x T
        for conv in self.conv_layers:
            hidden_states = conv(hidden_states)
            hidden_states = nn.functional.glu(hidden_states, dim=1)
        hidden_states = hidden_states.transpose(
            1, 2).contiguous()  # -> T x B x (C x D)
...
class Speech2TextPreTrainedModel(PreTrainedModel):
    ...
    def _get_subsampled_encoder_attn_mask(self, attention_mask):
        ...
        subsampled_lengths = self._get_subsampled_output_lengths(
        ...
        attention_mask[(torch.arange(
            bsz, device=attention_mask.device), subsampled_lengths - 1)] = 1
...
class Speech2TextEncoder(Speech2TextPreTrainedModel):
    ...
    def __init__(self, config: Speech2TextConfig):
        ...
        self.layers = nn.ModuleList([Speech2TextEncoderLayer(
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_mask = self._get_subsampled_encoder_attn_mask(
                attention_mask)
...
class Speech2TextDecoder(Speech2TextPreTrainedModel):
    ...
    def __init__(self, config: Speech2TextConfig):
        ...
        self.layers = nn.ModuleList([Speech2TextDecoderLayer(
...
class Speech2TextForConditionalGeneration(Speech2TextPreTrainedModel):
    ...
    def __init__(self, config: Speech2TextConfig):
        ...
        self.lm_head = nn.Linear(
            config.d_model, self.config.vocab_size, bias=False)
