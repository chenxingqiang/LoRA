...
class TFLEDEncoderAttention(tf.keras.layers.Layer):
    ...
    def call(self, inputs, training=False):
        ...
        attention_output = self.output_dense(
            self_outputs[0], training=training)
...
class TFLEDEncoderLayer(tf.keras.layers.Layer):
    def __init__(self, config: LEDConfig, layer_id: int, **kwargs):
        ...
        self.self_attn = TFLEDEncoderAttention(
            config, layer_id, name="self_attn")
...
class TFLEDEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: LEDConfig, embed_tokens: Optional[TFSharedEmbeddings] = None, **kwargs):
        ...
        self.layers = [TFLEDEncoderLayer(
            config, i, name=f"layers.{i}") for i in range(config.encoder_layers)]
        ...
        if inputs_embeds is not None:
            ...
            def pad_embeddings():
                input_ids_padding = tf.fill(
                    (batch_size, padding_len), pad_token_id)
...
class TFLEDModel(TFLEDPreTrainedModel):
    ...
    def serving_output(self, output):
        ...
        enc_g_attns = tf.convert_to_tensor(
            output.encoder_global_attentions) if self.config.output_attentions else None
