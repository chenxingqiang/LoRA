...
class TFBertPreTrainingLoss:
    ...
    def compute_loss(self, labels: tf.Tensor, logits: tf.Tensor) -> tf.Tensor:
        ...
        next_sentence_active_loss = tf.not_equal(tf.reshape(
            tensor=labels["next_sentence_label"], shape=(-1,)), -100)
...
class TFBertEncoder(tf.keras.layers.Layer):
    def __init__(self, config: BertConfig, **kwargs):
        ...
        self.layer = [TFBertLayer(config, name="layer_._{}".format(i))
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["labels"] is not None and inputs["next_sentence_label"] is not None:
            ...
            total_loss = self.compute_loss(labels=d_labels, logits=(
