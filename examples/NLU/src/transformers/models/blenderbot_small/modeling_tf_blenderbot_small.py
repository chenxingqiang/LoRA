...
class TFBlenderbotSmallEncoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: BlenderbotSmallConfig, embed_tokens: Optional[TFSharedEmbeddings] = None, **kwargs):
        ...
        self.layers = [TFBlenderbotSmallEncoderLayer(
    ...
    def call(
        ...
        **kwargs,
        ...
        if inputs["inputs_embeds"] is None:
            inputs["inputs_embeds"] = self.embed_tokens(
...
class TFBlenderbotSmallDecoder(tf.keras.layers.Layer):
    ...
    def __init__(self, config: BlenderbotSmallConfig, embed_tokens: Optional[TFSharedEmbeddings] = None, **kwargs):
        ...
        self.embed_scale = tf.math.sqrt(
            float(config.d_model)) if config.scale_embedding else 1.0
        self.layers = [TFBlenderbotSmallDecoderLayer(
    ...
    def call(
        ...
        **kwargs,
        ...
        hidden_states = self.layernorm_embedding(
            inputs["inputs_embeds"]) + positions
        hidden_states = self.dropout(
            hidden_states, training=inputs["training"])
...
class TFBlenderbotSmallMainLayer(tf.keras.layers.Layer):
    ...
    def __init__(self, config: BlenderbotSmallConfig, **kwargs):
        ...
        self.encoder = TFBlenderbotSmallEncoder(
            config, embed_tokens, name="encoder")
        self.decoder = TFBlenderbotSmallDecoder(
            config, embed_tokens, name="decoder")
