        ...
        if self.n_clusters == 0:
            ...
            for i in range(len(self.cutoffs)):
                ...
                if i == 0:
                    ...
                    head_logit = self._logit(
                        hidden, cur_W, cur_b, self.out_projs[0])
                    ...
                    tail_logit = self._logit(
                        hidden, cur_W, cur_b, self.out_projs[i])
                    ...
                    if target is not None:
                        ...
                        cur_logprob += cur_head_logprob[:,
                                                        self.cutoff_ends[1] + i - 1]
                if target is not None:
                    loss += tf.scatter_nd(mask_idx, -
                                          cur_logprob, shape_list(loss))
