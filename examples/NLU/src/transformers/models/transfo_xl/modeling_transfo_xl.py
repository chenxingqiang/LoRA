        ...
        if div_val == 1:
            ...
            if d_proj != d_embed:
                self.emb_projs.append(nn.Parameter(
                    torch.FloatTensor(d_proj, d_embed)))
        else:
            for i in range(len(self.cutoffs)):
                ...
                self.emb_projs.append(nn.Parameter(
                    torch.FloatTensor(d_proj, d_emb_i)))
...
class TransfoXLPreTrainedModel(PreTrainedModel):
            ...
            if hasattr(m, "emb_projs"):
                for i in range(len(m.emb_projs)):
                    if m.emb_projs[i] is not None:
                        nn.init.normal_(
                            m.emb_projs[i], 0.0, self.config.proj_init_std)
            ...
            if hasattr(m, "out_projs"):
                for i in range(len(m.out_projs)):
                    if m.out_projs[i] is not None:
                        nn.init.normal_(
                            m.out_projs[i], 0.0, self.config.proj_init_std)
    ...
    def resize_token_embeddings(self, new_num_tokens: Optional[int] = None, layer: Optional[int] = -1):
        ...
        model_embeds = base_model._resize_token_embeddings(
            new_num_tokens_layer, layer)
    ...
    def _get_new_num_tokens_layer(self, new_num_tokens, layer):
        ...
        new_num_tokens_layer = (
            new_num_tokens
            - sum([emb.weight.shape[0]
                  for emb in embeddings.emb_layers[:layer]])
            - sum([emb.weight.shape[0]
                  for emb in embeddings.emb_layers[layer + 1:]])
        )
...
class TransfoXLLMHeadModel(TransfoXLPreTrainedModel):
    ...
    def forward(
        ...
        prediction_scores = softmax_output.view(
            bsz, tgt_len, -1) if labels is None else ()
        loss = softmax_output.view(
            bsz, tgt_len - 1) if labels is not None else None
