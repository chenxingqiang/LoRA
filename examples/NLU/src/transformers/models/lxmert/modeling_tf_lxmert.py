...
class TFLxmertXLayer(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.lang_self_att = TFLxmertSelfAttentionLayer(
            config, name="lang_self_att")
        self.visn_self_att = TFLxmertSelfAttentionLayer(
            config, name="visn_self_att")
...
class TFLxmertEncoder(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        self.layer = [TFLxmertLayer(config, name="layer_._{}".format(i))
                      for i in range(self.num_l_layers)]
        self.x_layers = [TFLxmertXLayer(
            config, name="x_layers_._{}".format(i)) for i in range(self.num_x_layers)]
        self.r_layers = [TFLxmertLayer(config, name="r_layers_._{}".format(
            i)) for i in range(self.num_r_layers)]
        self.config = config
    ...
    def call(
        ...
        visual_feats = self.visn_fc(
            [visual_feats, visual_pos], training=training)
        ...
        **kwargs,
        ...
        if inputs["visual_attention_mask"] is not None:
            extended_visual_attention_mask = tf.reshape(
                inputs["visual_attention_mask"], (input_shape[0],
                                                  1, 1, input_shape[1])
            )
            ...
            extended_visual_attention_mask = tf.multiply(
                tf.subtract(
                    one_cst, extended_visual_attention_mask), ten_thousand_cst
            )
        else:
        ...
        hidden_states = (language_hidden_states,
                         vision_hidden_states) if inputs["output_hidden_states"] else ()
        ...
        return TFLxmertModelOutput(
            ...
            cross_encoder_attentions=cross_encoder_attentions if inputs[
...
class TFLxmertModel(TFLxmertPreTrainedModel):
    ...
    def serving_output(self, output):
        l_hs = tf.convert_to_tensor(
            output.language_hidden_states) if self.config.output_hidden_states else None
        v_hs = tf.convert_to_tensor(
            output.vision_hidden_states) if self.config.output_hidden_states else None
        l_attns = tf.convert_to_tensor(
            output.language_attentions) if self.config.output_attentions else None
        v_attns = tf.convert_to_tensor(
            output.vision_attentions) if self.config.output_attentions else None
        c_enc_attns = tf.convert_to_tensor(
            output.cross_encoder_attentions) if self.config.output_attentions else None
...
class TFLxmertLMPredictionHead(tf.keras.layers.Layer):
    def __init__(self, config: LxmertConfig, input_embeddings: tf.keras.layers.Layer, **kwargs):
        ...
        self.transform = TFLxmertPredictionHeadTransform(
...
class TFLxmertVisualObjHead(tf.keras.layers.Layer):
    def __init__(self, config, **kwargs):
        ...
        if config.visual_obj_loss:
            visual_losses["feat"] = {
                "shape": (-1, 2048), "num": config.visual_feat_dim}
...
class TFLxmertForPreTraining(TFLxmertPreTrainedModel):
    def __init__(self, config, *inputs, **kwargs):
        ...
        # Pre-training heads
        self.cls = TFLxmertPreTrainingHeads(
            config, self.lxmert.embeddings, name="cls")
        ...
        **kwargs,
        ...
        if inputs["obj_labels"] is not None and self.task_obj_predict:
            ...
            for key, key_info in self.visual_losses.items():
                ...
                visn_loss = tf.reduce_mean(
                    visn_loss * tf.cast(tf.reshape(mask_conf, [-1]), visn_loss.dtype)) * weight
                total_visn_loss += visn_loss
                losses += (visn_loss,)
            total_loss += total_visn_loss
        if inputs["ans"] is not None and self.task_qa:
            answer_loss = self.loss_fcts["ce"](
                tf.reshape(ans, [-1]), tf.reshape(answer_score,
                                                  [-1, self.num_qa_labels])
            )
