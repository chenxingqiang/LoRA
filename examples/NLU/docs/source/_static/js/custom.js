...
function addCustomFooter() {
    ...
    questionOrIssue.innerHTML =
        "Stuck? Read our <a href='https://medium.com/huggingface'>Blog posts</a> or <a href='https://github.com/huggingface/transformers'>Create an issue</a>"
...
function addVersionControl() {
    ...
    // Main classes and models are nested so we need to go deeper
        ...
        parts[versionIndex] == 'main_classes' ||
        parts[versionIndex] == 'model_doc' ||
        parts[versionIndex] == 'internal'
    ) {
...
function platformToggle() {
    ...
    const getFrameworkSpans = (filteredCodeBlock) => {
        ...
        if (pytorchSpanPosition < tensorflowSpanPosition) {
            ...
            const finalTensorflowSpanPosition = isPrompt
                ? tensorflowSpanPosition - promptSpanIdentifier.length
                : tensorflowSpanPosition

            pytorchSpans = spans.slice(
                pytorchSpanPosition + pytorchSpanIdentifier.length + 1,
                finalTensorflowSpanPosition
            )
            ...
            const finalPytorchSpanPosition = isPrompt
                ? pytorchSpanPosition - promptSpanIdentifier.length
                : pytorchSpanPosition

            tensorflowSpans = spans.slice(
                tensorflowSpanPosition + tensorflowSpanIdentifier.length + 1,
                finalPytorchSpanPosition
            )
    ...
    const updatedCodeBlocks = codeBlocks
        ...
        .filter(
            (codeBlock) =>
                codeBlock.innerText.includes(pytorchIdentifier) && codeBlock.innerText.includes(tensorflowIdentifier)
        )
        .map(getFrameworkSpans)

    updatedCodeBlocks.forEach(createFrameworkButtons)
}
    ...
    var e = window.document,
        ...
        x = function (e, t, o) {
            ...
            var u,
                ...
                x,
                y = e.appendChild(
                    r('div', { className: 'widget' + (/^large$/i.test(t['data-size']) ? ' lg' : '') }, [d])
                )
